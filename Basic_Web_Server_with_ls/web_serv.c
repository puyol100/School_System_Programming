...
int web_serv_info_print(char* path, char* home){
	...
	if(strcmp(path,fwd) ==0){ lflag =1;}
	else{aflag =1; lflag=1;}
		if(lflag ==1 && aflag ==0)
		{
		...
		while((dir = readdir(drip))!=NULL)
		{
			if(dir ->d_name[0] != '.')
			{
				...
				Head = sort_list(Head,dir,&file_info,0);
			}
		}
		Cur = Head;
		b_count =0;
		...
		{//print each information
		   fprintf(file,"<tr>");
			...
			{
                               // fprintf(file,"<td><a href =\"%s/%s\">%s</a></td>",fwd,Cur->name,Cur->name);
				fprintf(file,"<td><a href =\"%s\">%s</a></td>",Cur->name,Cur->name);
                        }
                        else 
			{
		...
		while(Cur!=NULL)
				...
				{
					fprintf(file,"<td><a href =\"%s\">%s</a></td>",tmp_url,Cur->name);
				}
				else if((strcmp(Cur ->name,"..") ==0))
				{
					...
					{
					fprintf(file,"<td><a href =\"%s\">%s</a></td>",tmp_url3,Cur->name);
					}
				}
				else
				{
